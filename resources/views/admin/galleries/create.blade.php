        ...
        <div class="card">
            <div class="card-body"> 
                <div class="alert alert-success" id="success-alert" style="display:none; text-align:center;">
                    ...
                    <strong style="text-align:center">Gallery Created Successfully!</strong>
                </div> 
                <h4 class="card-title">Create Gallery</h4>
                @if(Auth::guard('staff')->user())
                    <form id="gallery_create_form" class="form-sample" method="POST" action="{{ route('staff.galleries.store') }}" enctype="multipart/form-data">
                @elseif(Auth::guard('alumni')->user())
                    <form id="gallery_create_form" class="form-sample" method="POST" action="{{ route('alumni.galleries.store') }}" enctype="multipart/form-data">
                @else
                    <form id="gallery_create_form" class="form-sample" method="POST" action="{{ route('admin.galleries.store') }}" enctype="multipart/form-data">
                @endif
                    @csrf
                    ...
                    <div class="row my-3">
                        <div class="col-md-6">
                            <div class="row">
                            <label class="col-md-4" for="status">Event </label>
                            <select class="col-md-8" name="event_id" class="form-control" aria-label="Default select example">
